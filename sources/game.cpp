...
//------------------------------------------------------------------------------
static char GameVersionNumber[] = "VERSION 2.0.1";
...
void CGame::GameLoop()
{
	...
	if ( (m_pJoy1->next_level==1) || (m_pJoy2->next_level==1) )
	{
...
*/
	m_PlayerNameBuff1[0] = 'K';
	m_PlayerNameBuff1[1] = 'E';
	m_PlayerNameBuff1[2] = 'V';
	m_PlayerNameBuff1[3] = 'I';
	m_PlayerNameBuff1[4] = 0;//WAS cnt

	m_PlayerNameBuff2[0] = 'C';
	m_PlayerNameBuff2[1] = 'Y';
	m_PlayerNameBuff2[2] = 'N';
	m_PlayerNameBuff2[3] = 'D';

	m_PlayerNameBuff2[4] = 0;//WAS cnt
	PrepareEditName();
}
...
void CGame::GetPlayerNameLoop()
{
	...
	RedrawScrIfNeeded();
	if (m_EditPlayerOneNameFlag)
		EditName(m_pJoy1, nptr);
	else
		EditName(m_pJoy2, nptr);
}
...
void CGame::EditName(JOYSTICK *pjoy, char *nptr)
{
	char key;
	key = pjoy->key;
	if(pjoy->next_char==true&&pjoy->char_move!=0){
		m_HiOffset += pjoy->char_move;
		if (m_HiOffset<0) m_HiOffset = 3;
		if (m_HiOffset>=4) m_HiOffset = 0;
		pjoy->next_char = false;
	}
	if (key)
	{
		m_ScrChgFlag = 1;
